...
1. **AARC-360 logo file still needs to be added.** `includes/email_functions.php` now brands every outgoing email (dark teal header, lime accent bar, footer) and expects a real logo at `assets/images/aarc360-logo.png` in the Client Scheduler repo — falls back to a plain text "AARC-360" wordmark until that file exists. Garrett needs to save the actual image there, commit, push. Nothing else needs to change once it's added.
