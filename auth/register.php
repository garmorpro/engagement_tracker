...
// ---------------- STEP 1: GET -> CREATE REGISTRATION OPTIONS ----------------
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $excludeCredentials = array_map(fn($c)=>['type'=>'public-key','id'=>Base64UrlSafe::encodeUnpadded($c->getId())], $exclude);

    echo json_encode([
        'rp'=>['name'=>'Engagement Tracker','id'=>$_SERVER['HTTP_HOST']],
        'user'=>['id'=>Base64UrlSafe::encodeUnpadded($userHandle),'name'=>$accountName,'displayName'=>$accountName],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $userUUID = $_SESSION['webauthn_user_uuid'] ?? null;
    $userHandle = $_SESSION['webauthn_user_handle'] ?? null;
    ...
    $stmt->bind_param('ss', $userUUID, $data['account_name'] ?? 'Unnamed Account');
    $stmt->execute();
    $stmt->close();
